...
export default function MainContent({ children }) {
  return (
    <main className="flex-1 pt-24 min-h-screen bg-white relative overflow-hidden">
      {/* Decorative pastel blobs */}
      <div className="absolute top-10 left-10 w-72 h-72 bg-blue-200/40 rounded-full blur-3xl animate-pulse"></div>
      <div className="absolute bottom-20 right-20 w-80 h-80 bg-purple-200/30 rounded-full blur-3xl animate-pulse"></div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="relative z-10 max-w-7xl mx-auto w-full px-8 py-10"
        >
          <div className="bg-white rounded-2xl shadow-2xl p-8 min-h-[70vh] border border-gray-200">
            {children}
          </div>
        </motion.div>
      </AnimatePresence>
